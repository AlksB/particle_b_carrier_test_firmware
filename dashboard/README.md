...
| `geolocation` | `geolocation` (published as the device by the Logic in `cloud/geolocation`) | lat/lng/accuracy of the serving cell, tower key, cached or Google lookup |
...
cell the modem was camped on) and `device_position` (newest fix per device
and since when it has been on that tower; `fleet_now` carries the same
lat/lng).
...
file). Rows: Fleet (stat tiles + fleet-now table), Map (a Geomap of the newest
cell-tower fix per device, coloured by how long it has been silent, plus
the positions table and cached-vs-Google fixes per day), Connectivity (online
